...
class StartPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) => NonoPage(maxWidth: 640, children: [
    const NonoHeader(title: '노노링크', subtitle: '종이 신청서에서 시작해 안전한 이웃 매칭까지 연결합니다'),
    const SizedBox(height: 22),
    const NonoInfo(icon: '🩵', title: '안전한 재능 연결', body: '어르신은 손글씨 신청서로 재능을 등록하고, 앱에서는 명함 확인·매칭 수락·채팅만 이용합니다.', color: NonoTheme.coral),
    const SizedBox(height: 18),
    _RoleCard(emoji: '🏛️', title: '직원 / 봉사자', desc: '신청서 OCR 등록, 검수, 명함 발급', chips: const ['로그인', 'OCR 등록', '명함 발급'], page: const LoginExpertPage()),
    const SizedBox(height: 14),
    _RoleCard(emoji: '👵', title: '어르신', desc: '간단 인증, 내 명함 보기, 매칭 수락, 채팅', chips: const ['간단 인증', '내 명함', '채팅'], page: const SeniorLoginPage()),
    const SizedBox(height: 14),
    _RoleCard(emoji: '📱', title: '이웃 / 스캐너', desc: '로그인, QR 스캔, 매칭 신청, 후기 작성', chips: const ['신원 확인', 'QR 스캔', '후기'], page: const ScannerLoginPage()),
  ]);
}
